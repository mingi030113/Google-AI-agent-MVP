import { loadReferenceImages } from "./references/index.js";
...
export class GeminiVisionModelClient {
  ...
  async labelDefectType({ image, fields, process, selectedEquipment, patchcore, assetKey }) {
    if (!this.apiKey) {
      throw new Error("GEMINI_API_KEY is required for Gemini defect type labeling.");
    }

    const resolvedAssetKey = assetKey ?? fields?.assetKey ?? fields?.asset_key ?? null;
    const references = loadReferenceImages(resolvedAssetKey);
        ...
        parts: [
          ...
          },
          {
            text: [
              ...
              "{\"result\":\"normal|defective\",\"defectType\":\"scratch|contamination|dent|crack|flip|null\",\"confidence\":0.0,\"defectScores\":{\"scratch\":0.0,\"contamination\":0.0,\"dent\":0.0,\"crack\":0.0,\"flip\":0.0},\"reason\":\"short reason\"}",
              ...
              "Use flip when the product is reversed, upside down, incorrectly oriented, or its reference face/hole/chamfer direction does not match the normal sample.",
...
function buildLabelRequest({ image, fields, process, selectedEquipment, patchcore, assetKey, references = [] }) {
  const parts = [];

  if (references.length > 0) {
    parts.push({
      text: [
        `Product class: ${assetKey}.`,
        "Below are labeled REFERENCE images for this exact product class.",
        "Study them first to learn what a correctly-oriented (NORMAL) part looks like",
        "and what a FLIP defect looks like, then classify the final inspection image."
      ].join("\n")
    });

    references.forEach((reference, index) => {
      const tag = reference.label === "flip"
        ? "FLIP DEFECT (part is upside-down / back side showing)"
        : `${reference.label.toUpperCase()} (correct orientation)`;
      parts.push({ text: `Reference ${index + 1} - ${tag}${reference.note ? `: ${reference.note}` : ""}` });
      parts.push({
        inlineData: {
          mimeType: supportedMimeType(reference.mimeType),
          data: reference.base64
        }
      });
    });

    parts.push({ text: "Inspection image to classify:" });
  }
  ...
  const flipInstruction = references.length > 0
    ? "Compare the inspection image against the NORMAL reference images. In-plane rotation of the same face is NORMAL and must NOT be labeled flip. Choose flip only when the part is upside-down / its back side is showing like the FLIP reference (prongs curled the opposite way, threaded boss protruding around the hole, reference face/hole/chamfer direction reversed)."
    : "Use flip when the product is reversed, upside down, incorrectly oriented, or its reference face/hole/chamfer direction does not match the normal sample.";
  ...
  parts.push({
    text: [
      ...
      "Return only valid JSON with this exact shape:",
      "{\"defectTypeCandidate\":\"scratch|contamination|dent|crack|flip|null\",\"confidence\":0.0,\"defectScores\":{\"scratch\":0.0,\"contamination\":0.0,\"dent\":0.0,\"crack\":0.0,\"flip\":0.0},\"reason\":\"short reason\"}",
      "Use null only when no defect type can be inferred.",
      flipInstruction,
      ...assetSpecificLabelInstructions(assetKey),
      assetKey ? `Product class: ${assetKey}` : null,
...
function assetSpecificLabelInstructions(assetKey) {
  if (normalizeAssetKey(assetKey) !== "metal_nut") {
    return [];
  }

  return [
    "Metal_nut-specific labeling rules:",
    "- First decide whether the visible FACE/ORIENTATION matches the NORMAL references, before judging color or stain.",
    "- If the inspection image shows the opposite side/back side, reversed reference face, protruding center boss, opposite prong curl, reversed hole/chamfer direction, or a whole-part orientation mismatch, choose flip.",
    "- Do not choose contamination just because the flipped side has different brightness, texture, shadow, or color than the normal side.",
    "- Choose contamination only when there is a localized foreign material, residue, oil/glue mark, stain, or color patch sitting on the same correctly-oriented face.",
    "- When contamination and flip both seem plausible, prefer flip if the difference is global across the part face or centered on orientation features; prefer contamination only for local surface deposits."
  ];
}
...
function buildFallbackScores({ defectType, confidence, result }) {
  ...
  if (result === "defective" && defectType) {
    ...
    for (const type of ALLOWED_DEFECT_TYPES) {
      if (type !== defectType) {
        scores[type] = clampConfidence(Math.max(0.05, confidence * (secondaryWeights[offset] ?? 0.08)));
        offset += 1;
      }
    }
  }
